...
function checkTestEnv () {
  const argv = yargs(hideBin(process.argv))
    ...
    .example('$0 --network qie', 'run all tests using qie network')
    .example(
      '$0 --network qie --allowTests=test1,test2',
      'run only test1 and test2 using qie network'
    )
    .help('h')
    .alias('h', 'help')
    .describe('network', 'set which network to use: ganache|qie')
    .describe(
      'batch',
    ...
    .boolean('verbose-log')
    .describe('verbose-log', 'print qied output, default false').argv
  ...
  if (!argv.network) {
    ...
    if (argv.network !== 'qie' && argv.network !== 'ganache') {
      panic('network is invalid. Must be ganache or qie')
    } else {
...
function loadTests (runConfig) {
  ...
  fs.readdirSync(path.join(__dirname, 'suites')).forEach((dirname) => {
    ...
    try {
      ...
      const needScripts = ['test-ganache', 'test-qie']
...
function performTestSuite ({ testName, network }) {
  const cmd = network === 'ganache' ? 'test-ganache' : 'test-qie'
...
function setupNetwork ({ runConfig, timeout }) {
  if (runConfig.network !== 'qie') {
    // no need to start ganache. Truffle will start it
    return
  }

  // Spawn the qie process

  const spawnPromise = new Promise((resolve, reject) => {
    const serverStartedLog = 'Starting JSON-RPC server'
    const serverStartedMsg = 'qied started'

    const qiedProc = spawn('../e2e/init-node.sh', {
      cwd: __dirname,
      stdio: ['ignore', 'pipe', 'pipe']
    })

    logger.info(`Starting qied process... timeout: ${timeout}ms`)
    if (runConfig.verboseLog) {
      qiedProc.stdout.pipe(process.stdout)
    }

    qiedProc.stdout.on('data', (d) => {
  ...
  const timeoutPromise = new Promise((resolve, reject) => {
    setTimeout(() => reject(new Error('Start qied timeout!')), timeout)
  })

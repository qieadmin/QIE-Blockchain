...
// QieVersions is a custom comparator for sorting semver version strings.
type QieVersions []string

// Len is the number of stored versions.
func (v QieVersions) Len() int { return len(v) }

// Swap swaps the elements with indexes i and j. It is needed to sort the slice.
func (v QieVersions) Swap(i, j int) { v[i], v[j] = v[j], v[i] }

// Less compares semver versions strings properly
func (v QieVersions) Less(i, j int) bool {
...
func CheckUpgradeProposalVersion(version string) ProposalVersion {
	...
	cmp := QieVersions([]string{version, "v10.0.0", "v20.0.0"})
...
func RetrieveUpgradesList(upgradesPath string) ([]string, error) {
	...
	sort.Sort(QieVersions(versions))

	return versions, nil
}

// ExportState executes the  'docker cp' command to copy container .qied dir
...
func (m *Manager) ExportState(targetDir string) error {
	...
	cmd := exec.Command(
		"docker",
		"cp",
		fmt.Sprintf("%s:/root/.qied", m.ContainerID()),
		targetDir,
	)
	return cmd.Run()
}

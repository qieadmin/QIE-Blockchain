...
import (
	...
	testnetwork "github.com/puneet166/qie/testutil/integration/qie/network"
	...
	err = m.pool.Retry(
		func() error {
			...
			if c.State.ExitCode != 0 {
				stdOut, stdErr, _ := m.GetLogs(resource.Container.ID)
				return fmt.Errorf(
					"can't start qie node, container exit code: %d\n\n[error stream]:\n\n%s\n\n[output stream]:\n\n%s",
					c.State.ExitCode,
					stdErr,
					stdOut,
				)
...
// WaitForHeight queries the Qie node every second until the node will reach the specified height.
...
// GetNodeHeight calls the Qie CLI in the current node container to get the current block height
func (m *Manager) GetNodeHeight(ctx context.Context) (int, error) {
	cmd := []string{"qied", "q", "block"}
	...
	if !QieVersions([]string{m.CurrentVersion, "v20.0.0"}).Less(0, 1) {
	...
	if errBuff.String() != "" {
		return 0, fmt.Errorf("qie query error: %s", errBuff.String())
	}
...
// UnwrapBlockHeightPreV50 unwraps the block height from the output of the qied query block command
...
// UnwrapBlockHeightPostV50 unwraps the block height from the output of the qied query block command
...
// GetNodeVersion calls the Qie CLI in the current node container to get the
// current node version
func (m *Manager) GetNodeVersion(ctx context.Context) (string, error) {
	exec, err := m.CreateExec([]string{"qied", "version"}, m.ContainerID())
	if err != nil {
	...
	if errBuff.String() != "" {
		return "", fmt.Errorf("qie version error: %s", errBuff.String())
	}
...
func (m *Manager) getTimeoutCommit(ctx context.Context) (*big.Int, error) {
	exec, err := m.CreateExec([]string{"grep", `\s*timeout_commit =`, "/root/.qied/config/config.toml"}, m.ContainerID())
	if err != nil {
	...
	if errBuff.String() != "" {
		return common.Big0, fmt.Errorf("qie version error: %s", errBuff.String())
	}

...
def test_ibc_transfer_with_hermes(ibc):
    ...
    old_dst_balance = get_balance(ibc.chains["qie"], dst_addr, dst_denom)
    new_dst_balance = 0

    def check_balance_change():
        nonlocal new_dst_balance
        new_dst_balance = get_balance(ibc.chains["qie"], dst_addr, dst_denom)
    ...
    fee = int(events["tx"]["fee"].removesuffix("aqie"))
...
def test_qie_ibc_transfer_acknowledgement_error(ibc):
    """
    test sending aqie from qie to crypto-org-chain using cli

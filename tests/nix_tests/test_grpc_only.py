...
from .network import Qie, create_snapshots_dir, setup_custom_qie
from .utils import (
    CONTRACTS,
    decode_bech32,
    deploy_contract,
...
def grpc_eth_call(port: int, args: dict, chain_id=None, proposer_address=None):
    ...
    return requests.get(f"http://localhost:{port}/qie/evm/v1/eth_call", params).json()


def test_grpc_mode(qie_cluster: Qie):
    """
    ...
    api_port = ports.api_port(qie_cluster.base_port(1))
    ...
    for i in range(2):
        wait_for_block(qie_cluster.cosmos_cli(i), 1)
    supervisorctl(qie_cluster.base_dir / "../tasks.ini", "stop", "qie_1990-1-node1")

    # run grpc-only mode directly with existing chain state
    with open(qie_cluster.base_dir / "node1.log", "a", encoding="utf-8") as logfile:
        proc = subprocess.Popen(  # pylint: disable=consider-using-with
            [
                qie_cluster.chain_binary,
                "start",
                "--grpc-only",
                "--home",
                qie_cluster.base_dir / "node1",
            ],
            stdout=logfile,
        ...
        try:
            ...
            grpc_port = ports.grpc_port(qie_cluster.base_port(1))
            ...
            addr = qie_cluster.cosmos_cli(0).consensus_address()

...
from .network import setup_custom_qie, setup_qie
from .utils import (
    ADDRS,
    CONTRACTS,
    deploy_contract,
...
def custom_qie(tmp_path_factory):
    ...
    yield from setup_qie(path, 26200)


@pytest.fixture(scope="module")
def qie_indexer(tmp_path_factory):
...
@pytest.fixture(
    scope="module",
    params=["qie", "geth", "qie-ws", "qie-rocksdb", "enable-indexer"],
)
def cluster(request, custom_qie, qie_rocksdb, qie_indexer, geth):
    """
    run on both qie and geth
    """

...
from .ibc_utils import QIE_IBC_DENOM, assert_ready, get_balance, prepare_network
...
def test_ibc_transfer(ibc):
    ...
    old_src_balance = get_balance(ibc.chains["qie"], src_addr, src_denom)
    old_dst_balance = get_balance(ibc.chains["chainmain"], dst_addr, QIE_IBC_DENOM)
    ...
    gas_estimation = ibc.chains["qie"].w3.eth.estimate_gas(tx)
    receipt = send_transaction(ibc.chains["qie"].w3, tx, KEYS["signer2"])
    ...
    fee = receipt.gasUsed * qie_gas_price
    ...
    new_src_balance = get_balance(ibc.chains["qie"], src_addr, src_denom)
...
def test_ibc_transfer_invalid_packet(ibc):
    ...
    try:
        tx = pc.functions.transfer(
            ...
            0,
            "",
        ).build_transaction({"from": ADDRS["signer2"], "gasPrice": qie_gas_price})
        send_transaction(ibc.chains["qie"].w3, tx, KEYS["signer2"])
...
def test_staking(ibc):
    ...
    old_src_balance = get_balance(qie, del_addr, src_denom)
    ...
    tx = pc.functions.delegate(ADDRS["signer2"], validator_addr, amt).build_transaction(
        {"from": ADDRS["signer2"], "gasPrice": qie_gas_price}
    )
    gas_estimation = qie.w3.eth.estimate_gas(tx)
    ...
    new_src_balance = get_balance(qie, del_addr, src_denom)
    assert old_src_balance - amt - fee == new_src_balance
...
def test_staking_via_sc(ibc):
    ...
    approve_tx = contract.functions.testApprove(
        ...
        {"from": ADDRS["signer1"], "gasPrice": qie_gas_price, "gas": 60000}
    )

    gas_estimation = qie.w3.eth.estimate_gas(approve_tx)
    ...
    delegate_tx = contract.functions.testDelegate(
        ...
        {"from": ADDRS["signer1"], "gasPrice": qie_gas_price, "gas": 180000}
    )
    gas_estimation = qie.w3.eth.estimate_gas(delegate_tx)

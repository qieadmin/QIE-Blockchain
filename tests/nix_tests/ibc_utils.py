...
from .utils import (
    ...
    setup_stride,
    update_qie_bin,
    update_qied_and_setup_stride,
    wait_for_fn,
    wait_for_port,
)

# aqie IBC representation on another chain connected via channel-0.
QIE_IBC_DENOM = "ibc/8EAC8061F4499F03D2D1419A3E73D346289AE9DB89CAB1486B72539572B1915E"
# uosmo IBC representation on the Qie chain.
...
# uatom from cosmoshub-1 IBC representation on the Qie chain and on Cosmos Hub 2 chain.
...
EVM_CHAINS = ["qie_9000", "chainmain-1"]
...
def get_qie_generator(
    ...
    """
    setup qie with custom config
    depending on the build
    """
    post_init_func = None
    if is_rocksdb:
        file = memiavl_config(tmp_path, file)
        gen = setup_custom_qie(
            tmp_path,
            26710,
        ...
        if custom_scenario:
            ...
            modified_bin = build_patched_qied(custom_scenario)
            post_init_func = update_qie_bin(modified_bin)
            if stride_included:
                post_init_func = update_qied_and_setup_stride(modified_bin)
            gen = setup_custom_qie(
                tmp_path,
                26700,
                Path(__file__).parent / file,
...
def prepare_network(
    ...
    for chain in chain_names:
        ...
        # qie is the first chain
        # set it up and the relayer
        if "qie" in chain_name:
            # setup qie with the custom config
            # depending on the build
            gen = get_qie_generator(
                tmp_path,
                file,
                "-rocksdb" in chain,
            ...
            qie = next(gen)  # pylint: disable=stop-iteration-return

            # setup relayer
            hermes = Hermes(tmp_path / "relayer.toml")

            # wait for grpc ready
            wait_for_port(ports.grpc_port(qie.base_port(0)))  # qie grpc
            chains["qie"] = qie
            continue
    ...
    qie.supervisorctl("start", "relayer-demo")
...
def setup_denom_trace(ibc):
    """
    Helper setup function to send some funds from chain-main to qie
    to register the denom trace (if not registered already)
    """
    res = ibc.chains["qie"].cosmos_cli().denom_traces()
    if len(res["denom_traces"]) == 0:
        amt = 100
        src_denom = "basecro"
        dst_addr = ibc.chains["qie"].cosmos_cli().address("signer2")

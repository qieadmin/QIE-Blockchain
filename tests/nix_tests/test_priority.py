...
def custom_qie(tmp_path_factory):
    ...
    yield from setup_qie(path, 26800, long_timeout_commit=True)
...
def custom_qie_rocksdb(tmp_path_factory):
    ...
    # run with long timeout commit to ensure all
    # txs are included in the same block
    yield from setup_qie_rocksdb(path, 26810, long_timeout_commit=True)
...
def test_native_tx_priority(qie_cluster):
    ...
    test_cases = [
        {
            ...
            "amount": "1000aqie",
            "gas_prices": f"{base_fee + PRIORITY_REDUCTION * 600000}aqie",
        ...
        },
        {
            ...
            "amount": "1000aqie",
            "gas_prices": f"{base_fee + PRIORITY_REDUCTION * 400000}aqie",
    ...
    for tc in test_cases:
        ...
        gas_price = int(tc["gas_prices"].removesuffix("aqie"))

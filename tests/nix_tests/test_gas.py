...
def test_gas_eth_tx(geth, qie_cluster):
    tx_value = 10
    ...
    # send an equivalent transaction with qie
    qie_gas_price = qie_cluster.w3.eth.gas_price
    tx = {"to": ADDRS["community"], "value": tx_value, "gasPrice": qie_gas_price}
    qie_receipt = send_transaction(qie_cluster.w3, tx, KEYS["validator"])

    assert geth_receipt.gasUsed == qie_receipt.gasUsed


def test_gas_deployment(geth, qie_cluster):
    # deploy an identical contract on geth and qie
    ...
    qie_tx = build_deploy_contract_tx(qie_cluster.w3, info)

    # estimate tx gas
    geth_gas_estimation = geth.w3.eth.estimate_gas(geth_tx)
    qie_gas_estimation = qie_cluster.w3.eth.estimate_gas(qie_tx)

    assert geth_gas_estimation == qie_gas_estimation

    # sign and send tx
    geth_contract_receipt = send_transaction(geth.w3, geth_tx)
    qie_contract_receipt = send_transaction(qie_cluster.w3, qie_tx)
    assert geth_contract_receipt.status == 1
    assert qie_contract_receipt.status == 1

    assert geth_contract_receipt.gasUsed == qie_contract_receipt.gasUsed

    # gasUsed should be same as estimation
    assert geth_contract_receipt.gasUsed == geth_gas_estimation
    assert qie_contract_receipt.gasUsed == qie_gas_estimation


def test_gas_call(geth, qie_cluster):
    function_input = 10
    ...
    qie_contract, _ = deploy_contract(qie_cluster.w3, CONTRACTS["BurnGas"])
    ...
    # repeat the above for qie
    qie_gas_price = qie_cluster.w3.eth.gas_price
    qie_tx = qie_contract.functions.burnGas(function_input).build_transaction(
        {"from": ADDRS["validator"], "gasPrice": qie_gas_price}
    )
    qie_gas_estimation = qie_cluster.w3.eth.estimate_gas(qie_tx)
    qie_call_receipt = send_transaction(qie_cluster.w3, qie_tx)

    # ensure gas estimation is the same
    assert geth_gas_estimation == qie_gas_estimation

    # ensure that the gasUsed is equivalent
    assert geth_call_receipt.gasUsed == qie_call_receipt.gasUsed

    # ensure gasUsed == gas estimation
    assert geth_call_receipt.gasUsed == geth_gas_estimation
    assert qie_call_receipt.gasUsed == qie_gas_estimation


def test_block_gas_limit(qie_cluster):
    tx_value = 10

    # get the block gas limit from the latest block
    w3_wait_for_new_blocks(qie_cluster.w3, 5)
    block = qie_cluster.w3.eth.get_block("latest")
    ...
    try:
        send_transaction(qie_cluster.w3, tx, KEYS["validator"])
    except Exception as error:
        assert "exceeds block gas limit" in error.args[0]["message"]

    # deploy a contract on qie
    qie_contract, _ = deploy_contract(qie_cluster.w3, CONTRACTS["BurnGas"])

    # expect an error on contract call due to block gas limit
    try:
        burn_gas_tx = qie_contract.functions.burnGas(
            exceeded_gas_limit
        ).build_transaction(
            {
        ...
        send_transaction(qie_cluster.w3, burn_gas_tx, KEYS["validator"])
    except Exception as error:
        assert "exceeds block gas limit" in error.args[0]["message"]

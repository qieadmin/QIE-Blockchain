...
def ibc(request, tmp_path_factory):
    """
    prepare IBC network with an qie chain
...
def test_qie_ibc_transfer_native_denom(ibc, name, transfer_amt, err_contains):
    """
    test sending aqie from qie to crypto-org-chain using cli.
    """
    assert_ready(ibc)
    ...
    old_src_balance = get_balance(qie, src_addr, src_denom)
    old_dst_balance = get_balance(chainmain, dst_addr, QIE_IBC_DENOM)
...
def test_qie_ibc_transfer_ibc_denom(
    ...
    old_dst_balance = get_balance(qie, src_addr, src_denom)
    ...
    if err_inflow is not None:
        ...
        new_dst_balance = get_balance(qie, src_addr, src_denom)
        assert new_dst_balance == old_dst_balance
        return

    def check_balance_change_qie():
        new_dst_balance = get_balance(qie, src_addr, src_denom)
        return old_dst_balance < new_dst_balance

    wait_for_fn("balance change", check_balance_change_qie)

    # submit proposal if limit was not set
    limits = cli.rate_limits()
    # expect to already have one rate limit (for 'aqie') from the previous test
    if len(limits) == 1:
        add_rate_limit(qie, src_denom)
...
def add_rate_limit(qie: Qie, denom: str = "aqie"):
    cli = qie.cosmos_cli()

...
from .network import CosmosChain, Qie
from .utils import (
    ADDRS,
    KEYS,
    WQIE_ADDRESS,
...
# uatom from cosmoshub-2 -> cosmoshub-1 IBC representation on the Qie chain.
...
# The ERC20 address of ATOM on Qie
...
UPDATE_PARAMS_PROP = {
    "messages": [
        {
            "@type": "/qie.erc20.v1.MsgUpdateParams",
            "authority": "qie10d07y265gmmuvt4z0w9aw880jnsr700jcrztvm",
            "params": {
                "enable_erc20": True,
                "native_precompiles": [],
                "dynamic_precompiles": [],
...
def ibc(request, tmp_path_factory):
    ...
    network = prepare_network(path, name, [qie_build, "cosmoshub-1", "cosmoshub-2"])
    yield from network
...
def test_str_v2_single_hop(ibc):
    ...
    w3 = qie.w3
    qie_cli = qie.cosmos_cli()
    qie_addr = ADDRS["signer2"]
    bech_dst = eth_to_bech32(qie_addr)

    gaia_cli = gaia.cosmos_cli()
    gaia_addr = gaia_cli.address("signer2")

    # Before IBC transfer, check no dynamic precompiles available
    active_dynamic_precompiles = qie_cli.erc20_params()["params"][
        "dynamic_precompiles"
    ]
    assert len(active_dynamic_precompiles) == 0

    # Check token pairs before IBC transfer,
    # should only exist the WQIE pair
    pairs = qie_cli.get_token_pairs()
    assert len(pairs) == 1

    old_dst_balance = get_balance(qie, bech_dst, ATOM_IBC_DENOM)
    rsp = gaia_cli.ibc_transfer(
        gaia_addr, bech_dst, "5000uatom", "channel-0", 1, fees="10000uatom"
    )
    assert rsp["code"] == 0

    wait_for_ack(qie_cli, "Qie")

    pairs = qie_cli.get_token_pairs()
    active_dynamic_precompiles = qie_cli.erc20_params()["params"][
        "dynamic_precompiles"
    ]
    new_dest_balance = get_balance(qie, bech_dst, ATOM_IBC_DENOM)
    erc_dest_balance = erc20_balance(w3, ATOM_1_ERC20_ADDRESS, qie_addr)
...
def test_str_v2_multi_hop(ibc):
    ...
    qie_old_balance = get_balance(qie, bech_dst, ATOM_2_IBC_DENOM_MULTI_HOP)
    ...
    qie_balance = get_balance(qie, bech_dst, ATOM_2_IBC_DENOM_MULTI_HOP)
    dynamic_precompiles = qie_cli.erc20_params()["params"]["dynamic_precompiles"]
    token_pairs = qie_cli.get_token_pairs()

    # Here it's only one from the previous one we've registered in the first test
    assert qie_old_balance + 50000 == qie_balance
    assert len(dynamic_precompiles) == 1
    assert dynamic_precompiles[0] == ATOM_1_ERC20_ADDRESS
    assert len(token_pairs) == 2


def test_wqie_precompile_transfer(ibc):
    """
    Test the ERC20 transfer from one signer to another using the now
    registered ERC20 precompiled contract for WQIE.
    """
    assert_ready(ibc)
    ...
    w3 = qie.w3
    qie_balance = get_balance(qie, bech_dst, src_denom)
    signer2_balance = erc20_balance(w3, WQIE_ADDRESS, signer2)

    assert qie_balance == signer2_balance

    receipt = erc20_transfer(
        w3, WQIE_ADDRESS, signer1, signer2, 1000000, KEYS["signer1"]
    )
    assert receipt.status == 1

    signer_2_balance_after = erc20_balance(w3, WQIE_ADDRESS, signer2)
    assert signer_2_balance_after == signer2_balance + 1000000

    qie_balance_after = get_balance(qie, bech_dst, src_denom)
    assert qie_balance_after == qie_balance + 1000000
...
def test_toggle_erc20_precompile(ibc):
    ...
    acc = qie_cli.evm_account(contract_bech32_addr)
...
def update_erc20_params(qie: Qie, native_precomiles=[], dynamic_precompiles=[]):
    cli = qie.cosmos_cli()
    ...
    approve_proposal(qie, props[props_count - 1]["id"])
    wait_for_new_blocks(cli, 2)

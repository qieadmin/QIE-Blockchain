...
QIE_ADDRESS_PREFIX = "qie"
DEFAULT_DENOM = "aqie"
WQIE_ADDRESS = Web3.toChecksumAddress("0xD4949664cD82660AaE99bEdc034a0deA8A0bd517")
TEST_CONTRACTS = {
    ...
    "InterchainSender": "qie/testutil/contracts/InterchainSender.sol",
    "InterchainSenderCaller": "qie/testutil/contracts/InterchainSenderCaller.sol",
    "ICS20I": "qie/ics20/ICS20I.sol",
    "DistributionI": "qie/distribution/DistributionI.sol",
    "DistributionCaller": "qie/testutil/contracts/DistributionCaller.sol",
    "StakingI": "qie/staking/StakingI.sol",
    "StakingCaller": "qie/staking/testdata/StakingCaller.sol",
    "IStrideOutpost": "qie/outposts/stride/IStrideOutpost.sol",
    "IOsmosisOutpost": "qie/outposts/osmosis/IOsmosisOutpost.sol",
    "IERC20": "qie/erc20/IERC20.sol",
}

OSMOSIS_POOLS = {
    "Qie_Osmo": Path(__file__).parent / "osmosis/qieOsmosisPool.json",
}
...
REGISTER_ERC20_PROP = {
    "messages": [
        {
            "@type": "/qie.erc20.v1.MsgRegisterERC20",
...
def eth_to_bech32(addr, prefix=QIE_ADDRESS_PREFIX):
...
def update_node_cmd(path, cmd, i):
    ...
    for section in ini.sections():
        if section == f"program:qie_1990-1-node{i}":
            ini[section].update(
                {
...
def update_qied_and_setup_stride(modified_bin):
    def inner(path, base_port, config):  # pylint: disable=unused-argument
        update_qie_bin(modified_bin)(path, base_port, config)
...
def update_qie_bin(
    ...
    """
    updates the qie binary with a patched binary.
...
def debug_trace_tx(qie, tx_hash: str):
    url = f"http://127.0.0.1:{ports.evmrpc_port(qie.base_port(0))}"
    params = {

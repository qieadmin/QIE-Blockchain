...
def setup_qie(path, base_port, long_timeout_commit=False):
    ...
    yield from setup_custom_qie(path, base_port, cfg)
...
def create_snapshots_dir(
    ...
    for idx in range(n_nodes):
        data_snapshots_dir = path / "qie_1990-1" / f"node{idx}" / "data" / "snapshots"
        os.makedirs(data_snapshots_dir, exist_ok=True)


def setup_qie_rocksdb(path, base_port, long_timeout_commit=False):
    """
    setup_qie_rocksdb returns an Qie chain compiled with RocksDB
...
def setup_custom_qie(
    ...
    try:
        ...
        yield Qie(
            path / "qie_1990-1", chain_binary=chain_binary or DEFAULT_CHAIN_BINARY
        )
    finally:
        os.killpg(os.getpgid(proc.pid), signal.SIGTERM)
        proc.wait()
...
def build_patched_qied(patch_nix_file):
    """
    build the binary modified for a custom scenario
    e.g. allow to register WQIE token

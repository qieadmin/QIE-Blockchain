...
from .network import build_patched_qied, setup_custom_qie
from .utils import (
    supervisorctl,
    update_qie_bin,
    update_node_cmd,
...
def custom_qie(tmp_path_factory):
    ...
    broken_binary = build_patched_qied("broken-qied")
    print(broken_binary)
    ...
    yield from setup_custom_qie(
        ...
        post_init=update_qie_bin(broken_binary, [1]),
        wait_port=False,
    )


def test_rollback(custom_qie):
    """
    ...
    print("stop node1")
    supervisorctl(custom_qie.base_dir / "../tasks.ini", "stop", "qie_1990-1-node1")

    print("do rollback on node1")
    cli1.rollback()

    print("switch to normal binary")
    update_node_cmd(custom_qie.base_dir, "qied", 1)
    supervisorctl(custom_qie.base_dir / "../tasks.ini", "update")
    wait_for_port(target_port)

...
def custom_qie(tmp_path_factory):
    ...
    yield from setup_qie(path, 26500)


@pytest.fixture(scope="module")
def custom_qie_rocksdb(tmp_path_factory):
    path = tmp_path_factory.mktemp("fee-history-rocksdb")
    yield from setup_qie_rocksdb(path, 26510)


@pytest.fixture(scope="module", params=["qie", "qie-rocksdb", "geth"])
def cluster(request, custom_qie, custom_qie_rocksdb, geth):
    """
    run on qie, qie built with rocksdb and geth
    """

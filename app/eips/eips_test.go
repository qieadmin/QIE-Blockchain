...
import (
	...
	"github.com/puneet166/qie/app/eips/testdata"
	...
	integrationutils "github.com/puneet166/qie/testutil/integration/qie/utils"
...
func TestIPs(t *testing.T) {
	...
	RunSpecs(t, "QieIPs Suite")
}

var _ = Describe("Improvement proposal qie_0 - ", Ordered, func() {
	var (
		in network.Network
	...
	// Multiplier used to modify the opcodes associated with qie_0 IP.
	...
	It("should enable the new IP", func() {
		...
		qRes, err = gh.GetEvmParams()
		Expect(err).To(BeNil(), "failed during query to evm params")
		Expect(qRes.Params.ExtraEIPs).To(ContainElement(newIP), "expected to have IP qie_0 in evm params")
	})

	It("should change CREATE opcode constant gas after enabling qie_0 IP", func() {
...
var _ = Describe("Improvement proposal qie_1 - ", Ordered, func() {
	var (
		in network.Network
		tf factory.TxFactory
		gh grpc.Handler
	...
	// Multiplier used to modify the opcodes associated with qie_1.
	...
	It("should enable the new IP", func() {
		...
		qRes, err = gh.GetEvmParams()
		Expect(err).To(BeNil(), "failed during query to evm params")
		Expect(qRes.Params.ExtraEIPs).To(ContainElement(newIP), "expected to have ip qie_1 in evm params")
	})
...
var _ = Describe("Improvement proposal qie_2 - ", Ordered, func() {
	var (
		in network.Network
		tf factory.TxFactory
		gh grpc.Handler
	...
	// Constant gas used to modify the opcodes associated with qie_2.
	...
	It("should enable the new IP", func() {
		...
		qRes, err = gh.GetEvmParams()
		Expect(err).To(BeNil(), "failed during query to evm params")
		Expect(qRes.Params.ExtraEIPs).To(ContainElement(newIP), "expected to have ip qie_2 in evm params")
	})

...
func initTestnetFiles(
	...
	for i := 0; i < args.numValidators; i++ {
		...
		coins := sdk.Coins{
			sdk.NewCoin(qietypes.AttoQie, accStakingTokens),
		}
		...
		createValMsg, err := stakingtypes.NewMsgCreateValidator(
			addr.String(),
			valPubKeys[i],
			sdk.NewCoin(qietypes.AttoQie, valTokens),
		...
		customAppTemplate, customAppConfig := config.AppConfig(qietypes.AttoQie)
	...
	if err := initGenFiles(clientCtx, mbm, args.chainID, qietypes.AttoQie, genAccounts, genBalances, genFiles, args.numValidators); err != nil {
		return err
	}

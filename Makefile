...
HTTPS_GIT := https://github.com/puneet166/qie.git
...
# Deps
DEPS_COSMOS_SDK_VERSION := $(shell cat go.sum | grep -E 'github.com/evmos/cosmos-sdk/store\s' | grep -v -e 'go.mod' | tail -n 1 | awk '{ print $$2; }')
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=qie \
          -X github.com/cosmos/cosmos-sdk/version.AppName=$(QIE_BINARY) \
...
build-docker-goleveldb:
	...
	mkdir -p ./build/.qied
	echo '#!/usr/bin/env bash' > ./build/qied
	echo "IMAGE_NAME=${DOCKER_IMAGE}:${COMMIT_HASH}" >> ./build/qied
	echo 'SCRIPT_PATH=$$(cd $$(dirname $$0) && pwd -P)' >> ./build/qied
	echo 'docker run -it --rm -v $${SCRIPT_PATH}/.qied:/home/qie/.qied $$IMAGE_NAME qied "$$@"' >> ./build/qied
	chmod +x ./build/qied
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/puneet166/qie"
	godoc -http=:6060
...
proto-download-deps:
	...
	git init && \
	git remote add origin "https://github.com/evmos/cosmos-sdk/store.git" && \
...
PACKAGE_NAME:=github.com/puneet166/qie
...
# Qie repository and then clean up the contracts data.
...
# Compile the solidity contracts found in the Qie repository.
contracts-compile:

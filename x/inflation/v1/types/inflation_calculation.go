...
func CalculateEpochMintProvision(
	...
	// Multiply epochMintProvision with power reduction (10^18 for qie) as the
	// calculation is based on `qie` and the issued tokens need to be given in
	// `aqie`
	epochProvision = epochProvision.Mul(math.LegacyNewDecFromInt(qietypes.PowerReduction))

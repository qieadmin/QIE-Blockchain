	...
	testCases := []struct {
		...
		},
		{
			...
			map[string]func(*JumpTable){
				"qie_0": func(jt *JumpTable) {},
			},
			true,
			"",
			func() {
				eips := GetActivatorsEipNames()
				require.ElementsMatch(t, append(eips_snapshot, "qie_0"), eips, "expected eips number to be equal")
			},
		},
		{
			"success - multiple new activators",
			map[string]func(*JumpTable){
				"qie_1": func(jt *JumpTable) {},
				"qie_2": func(jt *JumpTable) {},
			},
			true,
			"",
			func() {
				...
				require.ElementsMatch(t, append(eips_snapshot, "qie_0", "qie_1", "qie_2"), eips, "expected eips number to be equal")
			},
		},
		{

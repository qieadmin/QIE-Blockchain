...
import (
	...
	"github.com/puneet166/qie/testutil/network"
...
Example:
	qied testnet init-files --v 4 --output-dir ./.testnets --starting-ip-address 192.168.10.2
	`,
	...
	cmd.Flags().String(flagNodeDaemonHome, "qied", "Home directory of the node's daemon configuration")
...
Example:
	qied testnet --v 4 --output-dir ./.testnets
	`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			args := startArgs{}
			args.outputDir, _ = cmd.Flags().GetString(flagOutputDir)
...
func initTestnetFiles(
	...
	if args.chainID == "" {
		args.chainID = fmt.Sprintf("qie_%d-1", cmtrand.Int63n(9999999999999)+1)
	}
	...
	for i := 0; i < args.numValidators; i++ {
		...
		kb, err := keyring.New(sdk.KeyringServiceName(), args.keyringBackend, nodeDir, inBuf, clientCtx.Codec, qiekr.Option())
		if err != nil {
			return err
		}
		...
		accStakingTokens := sdk.TokensFromConsensusPower(5000, qietypes.PowerReduction)
		coins := sdk.Coins{
		...
		valTokens := sdk.TokensFromConsensusPower(100, qietypes.PowerReduction)

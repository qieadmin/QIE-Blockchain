...
import (
	...
	qieclient "github.com/puneet166/qie/client"
	"github.com/puneet166/qie/client/block"
	"github.com/puneet166/qie/client/debug"
	qieserver "github.com/puneet166/qie/server"
	servercfg "github.com/puneet166/qie/server/config"
	srvflags "github.com/puneet166/qie/server/flags"

	"github.com/puneet166/qie/app"
	cmdcfg "github.com/puneet166/qie/cmd/config"
	qiekr "github.com/puneet166/qie/crypto/keyring"
)
...
// NewRootCmd creates a new root command for qied. It is called once in the
// main function.
func NewRootCmd() (*cobra.Command, sdktestutil.TestEncodingConfig) {
	...
	initClientCtx := client.Context{}.
		...
		WithKeyringOptions(qiekr.Option()).
		WithViper(EnvPrefix).
	...
	qieserver.AddCommands(
		rootCmd,
		qieserver.NewDefaultStartOptions(a.newApp, app.DefaultNodeHome),
		a.appExport,
		addModuleInitFlags,
	)
	...
	rootCmd.AddCommand(
		...
		queryCommand(),
		txCommand(),
		qieclient.KeyCommands(app.DefaultNodeHome),
	)
...
func (a appCreator) appExport(
	...
	if height != -1 {
		qieApp = app.NewQie(logger, db, traceStore, false, map[int64]bool{}, "", uint(1), appOpts)

		if err := qieApp.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		qieApp = app.NewQie(logger, db, traceStore, true, map[int64]bool{}, "", uint(1), appOpts)
	}

	return qieApp.ExportAppStateAndValidators(forZeroHeight, jailAllowedAddrs, modulesToExport)
}

...
import (
	...
	"github.com/puneet166/qie/crypto/hd"

	"github.com/puneet166/qie/server/config"
...
type Config struct {
	...
	GenesisState      qietypes.GenesisState // custom gensis state to provide
...
func DefaultConfig() Config {
	chainID := fmt.Sprintf("qie_%d-1", cmtrand.Int63n(9999999999999)+1)
	dir, err := os.MkdirTemp("", "simapp")
	if err != nil {
		panic(fmt.Sprintf("failed creating temporary directory: %v", err))
	}
	defer os.RemoveAll(dir)
	app := app.NewQie(log.NewNopLogger(), dbm.NewMemDB(), nil, true, nil, dir, 0, simutils.NewAppOptionsWithFlagHome(dir), baseapp.SetChainID(chainID))
	return Config{
		Codec:             app.AppCodec(),
		...
		NumValidators:     4,
		BondDenom:         "aqie",
		MinGasPrices:      fmt.Sprintf("0.000006%s", qietypes.AttoQie),
		AccountTokens:     sdk.TokensFromConsensusPower(1000000000000000000, qietypes.PowerReduction),
		StakingTokens:     sdk.TokensFromConsensusPower(500000000000000000, qietypes.PowerReduction),
		BondedTokens:      sdk.TokensFromConsensusPower(100000000000000000, qietypes.PowerReduction),
...
// NewAppConstructor returns a new Qie AppConstructor
...
func New(l Logger, baseDir string, cfg Config) (*Network, error) {
	...
	if !qietypes.IsValidChainID(cfg.ChainID) {
	...
	for i := 0; i < cfg.NumValidators; i++ {
		...
		nodeDir := filepath.Join(network.BaseDir, nodeDirName, "qied")
		clientDir := filepath.Join(network.BaseDir, nodeDirName, "qiecli")
		...
		genAccounts = append(genAccounts, &qietypes.EthAccount{
		...
		customAppTemplate, _ := config.AppConfig(qietypes.AttoQie)

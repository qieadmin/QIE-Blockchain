...
import (
	...
	evm "github.com/puneet166/qie/x/evm/types"
)
...
type ContractCallArgs struct {
	...
	Nonce *big.Int
	// Amount is the aqie amount to send in the transaction.
	Amount *big.Int
...
func DeployContract(
	...
	chainID := qieApp.EvmKeeper.ChainID()
	from := common.BytesToAddress(priv.PubKey().Address().Bytes())
	nonce := qieApp.EvmKeeper.GetNonce(ctx, from)
	...
	msgEthereumTx := evm.NewTx(&evm.EvmTxArgs{
		...
		Nonce:     nonce,
		GasLimit:  gas,
		GasFeeCap: qieApp.FeeMarketKeeper.GetBaseFee(ctx),
	...
	res, err := DeliverEthTx(qieApp, priv, msgEthereumTx)
	if err != nil {
		return common.Address{}, err
	}

	if _, err := CheckEthTxResponse(res, qieApp.AppCodec()); err != nil {
		return common.Address{}, err
	}
...
func DeployContractWithFactory(
	...
	chainID := qieApp.EvmKeeper.ChainID()
	from := common.BytesToAddress(priv.PubKey().Address().Bytes())
	factoryNonce := qieApp.EvmKeeper.GetNonce(ctx, factoryAddress)
	nonce := qieApp.EvmKeeper.GetNonce(ctx, from)

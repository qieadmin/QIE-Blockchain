...
// RegisterQieERC20Coins uses the UnitNetwork to register the qie token as an
...
func RegisterQieERC20Coins(
	...
	qieMetadata, found := network.App.BankKeeper.GetDenomMetaData(network.GetContext(), utils.BaseDenom)
	if !found {
		return erc20types.TokenPair{}, fmt.Errorf("expected qie denom metadata")
	}

	_, err = network.App.Erc20Keeper.RegisterERC20Extension(network.GetContext(), qieMetadata.Base)
	if err != nil {
		return erc20types.TokenPair{}, err
	}

	qieDenomID := network.App.Erc20Keeper.GetDenomMap(network.GetContext(), bondDenom)
	tokenPair, ok := network.App.Erc20Keeper.GetTokenPair(network.GetContext(), qieDenomID)
	if !ok {
		return erc20types.TokenPair{}, fmt.Errorf("expected qie erc20 token pair")
	}

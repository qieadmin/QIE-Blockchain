...
import (
	...
	"github.com/puneet166/qie/types"
	...
	commonnetwork "github.com/puneet166/qie/testutil/integration/common/network"
...
func (n *IntegrationNetwork) configureAndInitChain() error {
	...
	// Create a new QieApp with the following params
	qieApp := createQieApp(n.cfg.chainID, n.cfg.customBaseAppOpts...)
	...
	slashingParams, err := getValidatorsSlashingGen(validators, qieApp.StakingKeeper)
	if err != nil {
		return err
	}
	...
	genesisState, err = customizeGenesis(qieApp, n.cfg.customGenesisState, genesisState)
	if err != nil {
		return err
	}
	...
	header := cmtproto.Header{
		ChainID:            n.cfg.chainID,
		Height:             qieApp.LastBlockHeight() + 1,
		AppHash:            qieApp.LastCommitID().Hash,
		Time:               now,
	...
	if _, err := qieApp.FinalizeBlock(req); err != nil {
		return err
	}

	// TODO - this might not be the best way to initilize the context
	n.ctx = qieApp.BaseApp.NewContextLegacy(false, header)

	// Commit genesis changes
	if _, err := qieApp.Commit(); err != nil {
		return err
	}

...
var (
	...
	DefaultFee = sdk.NewCoin(utils.BaseDenom, sdkmath.NewIntFromUint64(uint64(feeAmt))) // 0.01 QIE
)
...
type CosmosTxArgs struct {
	...
	// ChainID is the chain's id on cosmos format, e.g. 'qie_1990-1'
	ChainID string
...
func signCosmosTx(
	...
	seq, err := appQie.AccountKeeper.GetSequence(ctx, addr)
	if err != nil {
		return nil, err
	}
	...
	accNumber := appQie.AccountKeeper.GetAccount(ctx, addr).GetAccountNumber()

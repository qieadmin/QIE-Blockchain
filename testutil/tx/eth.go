...
func PrepareEthTx(
	...
	signer := ethtypes.LatestSignerForChainID(appQie.EvmKeeper.ChainID())
	txFee := sdk.Coins{}
	txGasLimit := uint64(0)
...
func CreateEthTx(
	...
	chainID := appQie.EvmKeeper.ChainID()

	// When we send multiple Ethereum Tx's in one Cosmos Tx, we need to increment the nonce for each one.
	nonce := appQie.EvmKeeper.GetNonce(ctx, fromAddr) + uint64(nonceIncrement) //nolint:gosec // G115
	evmTxParams := &evmtypes.EvmTxArgs{
		...
		Amount:    amount,
		GasLimit:  100000,
		GasFeeCap: appQie.FeeMarketKeeper.GetBaseFee(ctx),

...
let
  ...
  ldflags = lib.concatStringsSep "\n" ([
    "-X github.com/cosmos/cosmos-sdk/version.Name=qie"
...
buildGoApplication rec {
  ...
  subPackages = [ "cmd/qied" ];
  CGO_ENABLED = "1";

  postFixup = if dbBackend == "rocksdb" then
    ''
      # Rename the binary from qied to qied-rocksdb
      mv $out/bin/qied $out/bin/qied-rocksdb
    '' else '''';

  meta = with lib; {
    description = "Qie is a scalable and interoperable blockchain, built on Proof-of-Stake with fast-finality using the Cosmos SDK which runs on top of CometBFT Core consensus engine.";
    homepage = "https://github.com/puneet166/qie";
    mainProgram = "qied";
  };
}

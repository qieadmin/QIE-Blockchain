...
func TestGetQieAddressFromBech32(t *testing.T) {
	testCases := []struct {
		name       string
		address    string
		expAddress string
		...
		{
			"qie address",
			"qie1qql8ag4cluz6r4dz28p3w00dnc9w8ueuafmxps",
			"qie1qql8ag4cluz6r4dz28p3w00dnc9w8ueuafmxps",
			false,
		},
		{
	...
	for _, tc := range testCases {
		addr, err := GetQieAddressFromBech32(tc.address)
		if tc.expError {
...
func TestQieCoinDenom(t *testing.T) {
	testCases := []struct {
		name     string
		denom    string
		expError bool

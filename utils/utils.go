...
const (
	// MainnetChainID defines the Qie EIP155 chain ID for mainnet
	MainnetChainID = "qie_2025"
	// TestnetChainID defines the Qie EIP155 chain ID for testnet
	TestnetChainID = "qie_2024"
	// TestingChainID defines the Qie EIP155 chain ID for integration test
	TestingChainID = "test_2024"
	// BaseDenom defines the Qie mainnet denomination
	BaseDenom = "aqie"
)
...
// IsMainnet returns true if the chain-id has the Qie mainnet EIP155 chain prefix.
func IsMainnet(chainID string) bool {
	return strings.HasPrefix(chainID, MainnetChainID)
}

// IsTestnet returns true if the chain-id has the Qie testnet EIP155 chain prefix.
...
// GetQieAddressFromBech32 returns the sdk.Account address of given address,
// while also changing bech32 human readable prefix (HRP) to the value set on
// the global sdk.Config (eg: `qie`).
// The function fails if the provided bech32 address is invalid.
func GetQieAddressFromBech32(address string) (sdk.AccAddress, error) {

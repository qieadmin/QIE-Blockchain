...
# The path to the main level of the Qie repository.
...
def is_qie_repo(path: Path) -> bool:
    """
    This function checks if the given path is the root of the Qie repository,
    ...
    with open(path / "go.mod", "r", encoding="utf-8") as go_mod:
        while True:
            ...
            if "module github.com/qie/qie" in line:
                return True
...
if __name__ == "__main__":
    if not is_qie_repo(REPO_PATH):
        raise ValueError(
            "This script should only be executed "
            + "in the qie repository."

...
## default port prefixes for qied
NODE_P2P_PORT="2660"
NODE_PORT="2663"
NODE_RPC_PORT="2666"
...
init_func() {
	"$PWD"/build/qied keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
	"$PWD"/build/qied init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
	"$PWD"/build/qied add-genesis-account \
		"$("$PWD"/build/qied keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000aqie,1000000000000000000stake \
		--keyring-backend test --home "$DATA_DIR$i"
	"$PWD"/build/qied gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
	"$PWD"/build/qied collect-gentxs --home "$DATA_DIR$i"
	"$PWD"/build/qied validate-genesis --home "$DATA_DIR$i"
...
start_func() {
	echo "starting qie node $i in background ..."
	"$PWD"/build/qied start --pruning=nothing --rpc.unsafe \
	...
	QIE_PID=$!
	echo "started qie node, pid=$QIE_PID"
	# add PID to array
	arr+=("$QIE_PID")
...
if [[ -z $TEST || $TEST == "rpc" || $TEST == "pending" ]]; then
	...
	for i in $(seq 1 "$TEST_QTD"); do
		...
		echo "going to test qie node $HOST_RPC ..."
...
stop_func() {
	QIE_PID=$i
	echo "shutting down node, pid=$QIE_PID ..."

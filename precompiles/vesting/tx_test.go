...
import (
	...
	"github.com/puneet166/qie/precompiles/vesting"
...
var (
	...
	toAddr           = qieutiltx.GenerateAddress()
	funderAddr       = qieutiltx.GenerateAddress()
	diffFunderAddr   = qieutiltx.GenerateAddress()
...
func (s *PrecompileTestSuite) TestCreateClawbackVestingAccount() {
	...
	testCases := []struct {
		...
		},
		{
			...
			malleate: func() []interface{} {
				differentAddr := qieutiltx.GenerateAddress()
				return []interface{}{
					funderAddr,
					differentAddr,
					false,
	...
	testCases := []struct {
		...
		},
		{
			...
			func() []interface{} {
				...
				err = qieutil.FundAccount(ctx, s.network.App.BankKeeper, toAddr.Bytes(), sdk.NewCoins(sdk.NewCoin(utils.BaseDenom, math.NewInt(100))))
				return []interface{}{
					s.keyring.GetAddr(0),
					toAddr,
					uint64(time.Now().Unix()), //nolint:gosec // G115
	...
	testCases := []struct {
		...
		},
		{
			...
			func(data []byte) {
				...
				_, ok := account.(qietypes.EthAccountI)
				s.Require().True(ok)
			},
			false,
			"",

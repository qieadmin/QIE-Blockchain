...
import (
	...
	auth "github.com/puneet166/qie/precompiles/authorization"
	"github.com/puneet166/qie/precompiles/erc20"
	"github.com/puneet166/qie/precompiles/erc20/testdata"
	...
	"github.com/puneet166/qie/testutil/integration/qie/utils"
	utiltx "github.com/puneet166/qie/testutil/tx"
...
var _ = Describe("ERC20 Extension -", func() {
	...
	BeforeEach(func() {
		...
		Expect(common.HexToAddress(erc20Params.NativePrecompiles[0])).To(Equal(common.HexToAddress(erc20types.WQIEContractTestnet)))

		wqieAddress = common.HexToAddress(erc20Params.NativePrecompiles[0])
	...
	Context("basic functionality -", func() {
		...
		When("calling reverter contract", func() {
			Context("in a direct call to the WQIE contract", func() {
				var (
				...
				DescribeTable("it should revert token transfer from the WQIE contract", func(before bool, after bool) {
					sender := is.keyring.GetKey(0)
					receiver := is.keyring.GetAddr(1)
					amountToSend := big.NewInt(100)
				...
				It("it should send token transfer and send from WQIE contract", func() {
					sender := is.keyring.GetKey(0)
					receiver := is.keyring.GetAddr(1)
				...
				DescribeTable("it should revert token transfer and send from WQIE contract", func(before bool, after bool) {
					sender := is.keyring.GetKey(0)
					receiver := is.keyring.GetAddr(1)
					balRes, err := is.handler.GetBalance(receiver.Bytes(), is.bondDenom)
					Expect(err).To(BeNil())
...
var _ = Describe("ERC20 Extension migration Flows -", func() {
	...
	When("using Qie (not wQie) in smart contracts", func() {
		It("should be using straight Qie for sending funds in smart contracts", func() {
			Skip("will be addressed in follow-up PR")

...
import (
	...
	compiledcontracts "github.com/puneet166/qie/contracts"
	"github.com/puneet166/qie/crypto/ethsecp256k1"
	"github.com/puneet166/qie/precompiles/authorization"
	cmn "github.com/puneet166/qie/precompiles/common"
	"github.com/puneet166/qie/precompiles/distribution"
	"github.com/puneet166/qie/precompiles/staking"
	"github.com/puneet166/qie/precompiles/staking/testdata"
	"github.com/puneet166/qie/precompiles/testutil"
	"github.com/puneet166/qie/precompiles/testutil/contracts"
	qieutil "github.com/puneet166/qie/testutil"
	"github.com/puneet166/qie/testutil/integration/qie/factory"
	"github.com/puneet166/qie/testutil/integration/qie/keyring"
	testutils "github.com/puneet166/qie/testutil/integration/qie/utils"
	testutiltx "github.com/puneet166/qie/testutil/tx"
...
var _ = Describe("Calling staking precompile directly", func() {
	...
	Describe("Execute approve transaction", func() {
		...
		It("should approve the undelegate method with 1 qie", func() {
			granter := s.keyring.GetKey(0)
			grantee := s.keyring.GetKey(1)
		...
		It("should approve the redelegate method with 2 qie", func() {
			granter := s.keyring.GetKey(0)
			grantee := s.keyring.GetKey(1)
		...
		It("should approve the cancel unbonding delegation method with 1 qie", func() {
			granter := s.keyring.GetKey(0)
			grantee := s.keyring.GetKey(1)
	...
	Describe("Execute increase allowance transaction", func() {
		...
		It("Should increase the allowance of the delegate method with 1 qie", func() {
			granter := s.keyring.GetKey(0)
			grantee := s.keyring.GetKey(1)
	...
	Describe("Execute decrease allowance transaction", func() {
		...
		It("Should decrease the allowance of the delegate method with 1 qie", func() {
			granteeAddr := s.precompile.Address()
			granter := s.keyring.GetKey(0)
	...
	Describe("Calling precompile txs from a vesting account", func() {
		...
		BeforeEach(func() {
			...
			vestingAmtTotal := qieutil.TestVestingSchedule.TotalVestingCoins
		...
		Context("before first vesting period - all tokens locked and unvested", func() {
			BeforeEach(func() {
				...
				Expect(unvested).To(Equal(qieutil.TestVestingSchedule.TotalVestingCoins), "expected different unvested coins")
			...
			It("Should be able to delegate tokens not involved in vesting schedule", func() {
				...
				Expect(err).To(BeNil())
				Expect(*balRes.Balance).To(Equal(accountGasCoverage[0].Add(qieutil.TestVestingSchedule.TotalVestingCoins[0]).Add(coinsToDelegate[0])))
				...
				Expect(err).To(BeNil())
				Expect(balRes.Balance.IsGTE(qieutil.TestVestingSchedule.TotalVestingCoins[0])).To(BeTrue())
			})
		})
		...
		Context("after first vesting period and before lockup - some vested tokens, but still all locked", func() {
			BeforeEach(func() {
				...
				cliffDuration := time.Duration(qieutil.TestVestingSchedule.CliffPeriodLength)
				Expect(s.network.NextBlockAfter(cliffDuration * time.Second)).To(BeNil())
				ctx := s.network.GetContext()

				// Check if some, but not all tokens are vested
				vested = clawbackAccount.GetVestedCoins(ctx.BlockTime())
				expVested := sdk.NewCoins(sdk.NewCoin(s.bondDenom, qieutil.TestVestingSchedule.VestedCoinsPerPeriod[0].Amount.Mul(math.NewInt(qieutil.TestVestingSchedule.CliffMonths))))
				Expect(vested).NotTo(Equal(qieutil.TestVestingSchedule.TotalVestingCoins), "expected some tokens to have been vested")
			...
			It("Should be able to delegate locked vested tokens + free tokens (not in vesting schedule)", func() {
				...
				balRes, err := s.grpcHandler.GetBalance(vestAcc.Bytes(), s.bondDenom)
				Expect(err).To(BeNil())
				Expect(*balRes.Balance).To(Equal(accountGasCoverage[0].Add(qieutil.TestVestingSchedule.TotalVestingCoins[0]).Add(amt[0])))
		...
		Context("Between first and second lockup periods - vested coins are unlocked", func() {
			BeforeEach(func() {
				...
				vestDuration := time.Duration(qieutil.TestVestingSchedule.LockupPeriodLength)
				...
				expVested := sdk.NewCoins(sdk.NewCoin(s.bondDenom, qieutil.TestVestingSchedule.VestedCoinsPerPeriod[0].Amount.Mul(math.NewInt(qieutil.TestVestingSchedule.LockupMonths))))
				expUnlockedVested := expVested

				Expect(vested).NotTo(Equal(qieutil.TestVestingSchedule.TotalVestingCoins), "expected not all tokens to be vested")
				...
				Expect(unlocked).To(Equal(qieutil.TestVestingSchedule.UnlockedCoinsPerLockup))
	...
	Context("delegating", func() {
		...
		Describe("delegation from a vesting account", func() {
			...
			BeforeEach(func() {
				...
				// Check if all tokens are unvested at vestingStart
				totalVestingCoins := qieutil.TestVestingSchedule.TotalVestingCoins
				unvested = clawbackAccount.GetVestingCoins(ctx.BlockTime())
				vested = clawbackAccount.GetVestedCoins(ctx.BlockTime())
			...
			Context("before first vesting period - all tokens locked and unvested", func() {
				BeforeEach(func() {
					...
					Expect(unvested).To(Equal(qieutil.TestVestingSchedule.TotalVestingCoins))
			...
			Context("after first vesting period and before lockup - some vested tokens, but still all locked", func() {
				BeforeEach(func() {
					...
					expVested := sdk.NewCoins(sdk.NewCoin(s.bondDenom, qieutil.TestVestingSchedule.VestedCoinsPerPeriod[0].Amount.Mul(math.NewInt(qieutil.TestVestingSchedule.CliffMonths))))
					Expect(vested).NotTo(Equal(qieutil.TestVestingSchedule.TotalVestingCoins))

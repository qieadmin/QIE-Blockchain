...
import (
	...
	qieutiltx "github.com/puneet166/qie/testutil/tx"
)

func (s *PrecompileTestSuite) TestBalances() {
	var ctx sdk.Context
	// setup test in order to have s.precompile, s.qieAddr and s.xmplAddr defined
	s.SetupTest()
	...
	testcases := []struct {
		...
		expPass     bool
		errContains string
		expBalances func(qieAddr, xmplAddr common.Address) []bank.Balance
	}{
		{
		...
		},
		{
			...
			true,
			"",
			func(qieAddr, xmplAddr common.Address) []bank.Balance {
				return []bank.Balance{
					{
						ContractAddress: qieAddr,
		...
		{
			"pass - QIE and XMPL balances present - mint extra XMPL",
			func() []interface{} {
	...
	for _, tc := range testcases {
		...
		s.Run(tc.name, func() {
			...
			if tc.expPass {
				...
				s.Require().NoError(err)
				s.Require().Equal(tc.expBalances(s.qieAddr, s.xmplAddr), balances)
			} else {
...
func (s *PrecompileTestSuite) TestTotalSupply() {
	...
	s.Require().NoError(err)
	qieTotalSupply := totSupplRes.Supply.AmountOf(s.bondDenom)
	xmplTotalSupply := totSupplRes.Supply.AmountOf(s.tokenDenom)

	testcases := []struct {
		name      string
		malleate  func()
		expSupply func(qieAddr, xmplAddr common.Address) []bank.Balance
	}{
		{
			"pass - QIE and XMPL total supply",
			func() {
				ctx = s.mintAndSendXMPLCoin(ctx, s.keyring.GetAccAddr(0), math.NewInt(1e18))
			},
			func(qieAddr, xmplAddr common.Address) []bank.Balance {
	...
	for _, tc := range testcases {
		...
		s.Run(tc.name, func() {
			...
			err = s.precompile.UnpackIntoInterface(&balances, method.Name, bz)
			s.Require().NoError(err)
			s.Require().Equal(tc.expSupply(s.qieAddr, s.xmplAddr), balances)
		})
	}
}

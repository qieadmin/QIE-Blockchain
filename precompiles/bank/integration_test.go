...
import (
	...
	"github.com/puneet166/qie/precompiles/bank"
	"github.com/puneet166/qie/precompiles/bank/testdata"
...
type IntegrationTestSuite struct {
	...
	qieAddr, xmplAddr   common.Address
...
func (is *IntegrationTestSuite) SetupTest() {
	...
	is.qieAddr = common.HexToAddress(tokenPair.Erc20Address)
...
var _ = Describe("Bank Extension -", func() {
	var (
		...
		qieTotalSupply, _ = new(big.Int).SetString("200003000000000000000000", 10)
	...
	Context("Direct precompile queries", func() {
		...
		Context("totalSupply query", func() {
			It("should return the correct total supply", func() {
				...
				Expect(balances[0].Amount).To(Equal(qieTotalSupply))
				Expect(balances[1].Amount).To(Equal(xmplTotalSupply))
			})
		})

		Context("supplyOf query", func() {
			It("should return the supply of Qie", func() {
				queryArgs, supplyArgs := getTxAndCallArgs(directCall, contractData, bank.SupplyOfMethod, is.qieAddr)
				_, ethRes, err := is.factory.CallContractAndCheckLogs(sender.Priv, queryArgs, supplyArgs, passCheck)
				Expect(err).ToNot(HaveOccurred(), "unexpected result calling contract")
				...
				Expect(out[0].(*big.Int)).To(Equal(qieTotalSupply))
			})
	...
	Context("Calls from a contract", func() {
		...
		Context("supplyOf query", func() {
			It("should return the supply of Qie", func() {
				queryArgs, supplyArgs := getTxAndCallArgs(contractCall, contractData, SupplyOfFunction, is.qieAddr)
				_, ethRes, err := is.factory.CallContractAndCheckLogs(sender.Priv, queryArgs, supplyArgs, passCheck)
				Expect(err).ToNot(HaveOccurred(), "unexpected result calling contract")
